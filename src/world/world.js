...
// 堤山 (羽床富士)。讃岐七富士のひとつ ── ほんとうに あるいて のぼれる山
export const TSUTSUMI = { x: -60, z: -165, h: 30, r: 52 };
// みなみの やまなみ (こちらも ほんものの地形。ふもとまで あるいて入れる)
export const SHILLS = [
  { x: -140, z: -152, h: 12, r: 38 },
  { x: 62, z: -154, h: 14, r: 40 },
  { x: 155, z: -147, h: 10, r: 32 },
];
...
// 大きな地形 (綾上の丘陵・山ぎわ・十瓶山・堤山とみなみの山なみ)
function ridges(x, z) {
  let y = 0;
  ...
  const dS = Math.hypot(x - TSUTSUMI.x, z - TSUTSUMI.z);
  y += TSUTSUMI.h * Math.exp(-(dS * dS) / (TSUTSUMI.r * TSUTSUMI.r));
...
export function groundY(x, z) {
  ...
  // 滝つぼ: 山はらを たいらに けずった たなの上に、あさい ふちが くぼむ
  const dF = Math.hypot(x - TAKIBO.x, z - TAKIBO.z);
  if (dF < TAKIBO.r + 5) {
    const t = 0.5 * (1 + Math.cos(Math.PI * Math.min(1, Math.max(0, dF - TAKIBO.r) / 5)));
    y = y * (1 - t) + TAKIBO.shelf * t;
    if (dF < TAKIBO.r) y -= 0.75 * 0.5 * (1 + Math.cos((Math.PI * dF) / TAKIBO.r));
  }
  return y;
}
...
export const roadDefs = [
  ...
  // 堤山: ひみつきちの竹やぶ横から 沢ぞいに滝つぼへ、つづらおりで さんちょうへ
  { pts: [[-22, -84], [-28, -98], [-33, -112], [-36, -121], [-42, -126]], w: 1.9 },
  { pts: [[-42, -126], [-52, -130], [-60, -136], [-66, -144], [-64, -152], [-56, -152], [-48, -155], [-52, -161], [-60, -165]], w: 1.7 },
  // 滝つぼへの わきみち
  { pts: [[-39, -124.5], [-36, -125.5]], w: 1.4 },
  // 陶: 山すその墓地への小道
...
export function buildWorld(scene) {
  ...
  const rockCol = new THREE.Color(0x7d7a6e);
  for (let i = 0; i < gp.count; i++) {
    ...
    // 基本は草の濃淡。標高が上がると山の濃い緑 → さんちょう近くは岩まじりに
    c.setHSL(0.24 + n * 0.03, 0.32 + n * 0.14, Math.max(0.28, 0.42 + n * 0.1 - y * 0.012));
    if (y > 22) c.lerp(rockCol, Math.min(1, (y - 22) / 10) * 0.38);
    if (carve > 0.5) c.setHSL(0.11, 0.3, 0.52 + n * 0.06); // 川原の砂
    else if (carve > 0.18) c.lerp(dirtCol, 0.55);
    // 滝つぼのまわりは ぬれた岩はだ
    if (Math.hypot(x - TAKIBO.x, z - TAKIBO.z) < TAKIBO.r + 1.6) c.setHSL(0.55, 0.12, 0.42 + n * 0.05);
  ...
  // ---------- 里山 (きたのはしの ちいさな山。みなみがわは ほんものの地形にかわった) ----------
  const satoyamaDefs = [
  ...
  // ---------- 堤山 (羽床富士) ── あるいて のぼれる ほんものの山 ----------
  // 山の木々 (みちと沢と滝つぼを さけて、松と広葉樹を まばらに)
  const mountainWoods = [
    { c: TSUTSUMI, n: 52 }, { c: SHILLS[0], n: 13 }, { c: SHILLS[1], n: 15 }, { c: SHILLS[2], n: 10 },
  ];
  let mti = 0;
  for (const mw of mountainWoods) {
    for (let k = 0; k < mw.n; k++) {
      mti++;
      const a = hash(mti, 301) * Math.PI * 2;
      const d = mw.c.r * (0.16 + hash(mti, 302) * 0.92);
      const hx = mw.c.x + Math.cos(a) * d, hz = mw.c.z + Math.sin(a) * d;
      if (Math.abs(hx) > 228 || hz > 145 || hz < -208) continue;
      if (roadDist(hx, hz) < 2.1) continue; // 山みちは ふさがない
      if (Math.hypot(hx - TAKIBO.x, hz - TAKIBO.z) < TAKIBO.r + 5.5) continue; // 滝つぼの ひろば
      if (Math.hypot(hx - TSUTSUMI.x, hz - TSUTSUMI.z) < 8) continue; // さんちょうの ひろば
      if (Math.abs(hz - riverCenterZ(hx)) < 10) continue;
      const t = hash(mti, 303) < 0.62 ? makePine(mti) : makeTree(hash(mti, 304) < 0.25, mti);
      t.position.set(hx, gy(hx, hz) - 0.12, hz);
      scene.add(t);
      addCircle(hx, hz, 0.55);
      if (hash(mti, 303) >= 0.62) world.trees.push({ x: hx, z: hz, big: hash(mti, 304) < 0.25 });
    }
  }
  // 山はだの岩
  const mrocks = new THREE.InstancedMesh(new THREE.DodecahedronGeometry(0.6, 0), smat(0x8a877c), 24);
  for (let i = 0; i < 24; i++) {
    const a = hash(i, 311) * Math.PI * 2;
    const d = TSUTSUMI.r * (0.12 + hash(i, 312) * 0.55);
    const rx = TSUTSUMI.x + Math.cos(a) * d, rz = TSUTSUMI.z + Math.sin(a) * d;
    dummy.position.set(rx, gy(rx, rz) + 0.12, rz);
    dummy.rotation.set(hash(i, 313) * 3, hash(i, 314) * 3, 0);
    dummy.scale.setScalar(0.5 + hash(i, 315) * 1.5);
    dummy.updateMatrix();
    mrocks.setMatrixAt(i, dummy.matrix);
  }
  scene.add(mrocks);
  // 道標 (ふもとと 滝つぼの わかれみち)
  const mich1 = makeMichishirube([{ text: 'つつみやま', side: -1 }, { text: 'たきのみや', side: 1 }]);
  mich1.position.set(-19.5, gy(-19.5, -83), -83);
  mich1.rotation.y = 0.5;
  scene.add(mich1);
  addCircle(-19.5, -83, 0.25);
  const mich2 = makeMichishirube([{ text: 'たきつぼ', side: 1 }, { text: 'さんちょう', side: -1 }]);
  mich2.position.set(-41, gy(-41, -122.5), -122.5);
  mich2.rotation.y = -0.4;
  scene.add(mich2);
  addCircle(-41, -122.5, 0.25);

  // ---------- 沢と滝と滝つぼ (あゆのすむ きれいな水) ----------
  // 滝つぼの水面 (あさい。じゃぶじゃぶ 入れる)
  const poolWater = new THREE.Mesh(new THREE.CircleGeometry(TAKIBO.r + 0.5, 26), smat(0x66aabf, { transparent: true, opacity: 0.72 }));
  poolWater.rotation.x = -Math.PI / 2;
  poolWater.position.set(TAKIBO.x, TAKIBO.y, TAKIBO.z);
  scene.add(poolWater);
  // 滝 (ながれおちる水のカーテン)
  const fallMat = new THREE.MeshLambertMaterial({ map: waterTex(), transparent: true, opacity: 0.85, emissive: 0x1c3a48 });
  const fall = new THREE.Mesh(new THREE.PlaneGeometry(2.6, 4.6), fallMat);
  fall.position.set(TAKIBO.x, TAKIBO.y + 1.7, TAKIBO.z - 3.35);
  fall.rotation.x = -0.09; // すこし まえかがみに おちる
  scene.add(fall);
  world.fallMat = fallMat;
  // 滝しぶきの白いあわ
  const foam = new THREE.Mesh(new THREE.CircleGeometry(1.5, 18), smat(0xe8f2f4, { transparent: true, opacity: 0.6 }));
  foam.rotation.x = -Math.PI / 2;
  foam.position.set(TAKIBO.x, TAKIBO.y + 0.02, TAKIBO.z - 3.1);
  scene.add(foam);
  world.foamMat = foam.material;
  // 滝をかこむ 大岩 (うえには のぼれない)
  for (const [rx, rz, rs] of [[-35.7, -131.4, 2.1], [-30.3, -131.2, 1.9], [-33, -132.8, 2.5], [-37.9, -129.6, 1.4], [-28.3, -129.4, 1.3]]) {
    const rk = new THREE.Mesh(new THREE.DodecahedronGeometry(rs, 0), smat(0x77746a));
    rk.position.set(rx, gy(rx, rz) + rs * 0.3, rz);
    rk.rotation.set(hash(rx, rz) * 3, hash(rz, rx) * 3, 0);
    scene.add(rk);
    addCircle(rx, rz, rs * 0.75);
  }
  // 沢 (滝のうえの ながれと、滝つぼから ふもとへ くだる ながれ)
  world.sawaMats = [];
  for (const spts of [
    [[-51, -157], [-45, -151], [-40, -146], [-35, -138], [-33.4, -133.4]],
    [[-31.8, -122.8], [-29.5, -113], [-26.5, -101], [-24, -92], [-22.5, -86.5]],
  ]) {
    const sawa = strip(densify(spts, 2), 1.0, 0.07, 0xffffff);
    sawa.material.map = waterTex();
    sawa.material.transparent = true;
    sawa.material.opacity = 0.85;
    scene.add(sawa);
    world.sawaMats.push(sawa.material);
  }
  // あゆ (滝つぼのぬし。3びき。うごきは events がつける)
  world.ayu = [];
  for (let i = 0; i < 3; i++) {
    const fishG = new THREE.Group();
    const body = new THREE.Mesh(new THREE.SphereGeometry(0.3, 8, 6), smat(0x46565e, { transparent: true, opacity: 0.88 }));
    body.scale.set(1.5, 0.38, 0.55);
    const tail = new THREE.Mesh(new THREE.ConeGeometry(0.11, 0.28, 4), smat(0x3c4a52, { transparent: true, opacity: 0.88 }));
    tail.rotation.z = Math.PI / 2;
    tail.position.x = -0.52;
    fishG.add(body, tail);
    fishG.position.set(TAKIBO.x + Math.cos(i * 2.1) * 1.8, TAKIBO.y - 0.08, TAKIBO.z + Math.sin(i * 2.1) * 1.8);
    fishG.visible = false;
    scene.add(fishG);
    world.ayu.push(fishG);
  }

  // ---------- さんちょう (見晴らし岩・ほこら・しるべ) ----------
  const iwID = new THREE.Mesh(new THREE.DodecahedronGeometry(1.9, 0), smat(0x8d8a7e));
  iwID.position.set(TSUTSUMI.x, gy(TSUTSUMI.x, TSUTSUMI.z - 1.5) + 0.5, TSUTSUMI.z - 1.5);
  iwID.scale.set(1.35, 0.72, 1.1);
  iwID.rotation.y = 0.7;
  scene.add(iwID);
  addCircle(TSUTSUMI.x, TSUTSUMI.z - 1.5, 1.8);
  const hokora = new THREE.Group();
  const hb1 = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.5, 0.7), smat(0x9a958c));
  hb1.position.y = 0.25;
  const hb2 = new THREE.Mesh(new THREE.BoxGeometry(0.68, 0.6, 0.55), smat(0x8a857a));
  hb2.position.y = 0.8;
  const hb3 = new THREE.Mesh(new THREE.ConeGeometry(0.62, 0.42, 4), smat(0x6a655c));
  hb3.position.y = 1.31;
  hb3.rotation.y = Math.PI / 4;
  hokora.add(hb1, hb2, hb3);
  hokora.position.set(-64.5, gy(-64.5, -163), -163);
  hokora.rotation.y = 0.6;
  scene.add(hokora);
  addCircle(-64.5, -163, 0.7);
  const summitSign = makeMichishirube([{ text: 'つつみやま 201m', side: 1 }]);
  summitSign.position.set(-56.5, gy(-56.5, -162.5), -162.5);
  summitSign.rotation.y = 0.3;
  scene.add(summitSign);
  addCircle(-56.5, -162.5, 0.25);
  world.summit = { x: -58.5, z: -163.8 };

  // ---------- 生活感: ものほしざお (はれた ひるまだけ せんたくものが 干される) ----------
  const makeLaundry = (cols) => {
    const g = new THREE.Group();
    for (const px of [-1.55, 1.55]) {
      const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 1.8, 5), smat(0x8a8578));
      pole.position.set(px, 0.9, 0);
      g.add(pole);
    }
    const bar = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 3.1, 5), smat(0xc8b890));
    bar.rotation.z = Math.PI / 2;
    bar.position.y = 1.62;
    g.add(bar);
    cols.forEach((cl, i) => {
      const sh = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.68, 0.05), smat(cl));
      sh.position.set(-1.05 + i * 0.72, 1.24, 0);
      g.add(sh);
    });
    return g;
  };
  world.laundry = [];
  for (const ls of [
    { x: 112.5, z: 16.5, ry: 0.2, cols: [0xf0ede4, 0x7ba3d8, 0xe8c8c0, 0xf0ede4] }, // ばあちゃんち
    { x: 38.7, z: 17, ry: -0.3, cols: [0xd8e0ea, 0xf0e6b0, 0xc8d8c0] }, // ミナんち
    { x: -144.5, z: -16.5, ry: 0.15, cols: [0xffffff, 0x88a8c8, 0xe0d8c8] }, // 綾上の農家
  ]) {
    const g = makeLaundry(ls.cols);
    g.position.set(ls.x, gy(ls.x, ls.z), ls.z);
    g.rotation.y = ls.ry;
    scene.add(g);
    world.laundry.push(g);
  }
  // 生活感: えんがわの ふうりん (Eで ちりんと ならせる)
  const furin = new THREE.Group();
  const fCord = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.5, 4), smat(0x555555));
  fCord.position.y = 0.25;
  const fBell = new THREE.Mesh(new THREE.ConeGeometry(0.13, 0.16, 10), smat(0x7ac0d8, { transparent: true, opacity: 0.85 }));
  fBell.position.y = 0;
  const fTan = new THREE.Mesh(new THREE.PlaneGeometry(0.09, 0.26), smat(0xfefefe, { side: THREE.DoubleSide }));
  fTan.position.y = -0.24;
  furin.add(fCord, fBell, fTan);
  furin.position.set(115.4, gy(115.4, 14.2) + 2.42, 14.2);
  scene.add(furin);
  world.furin = furin;
