...
export class UI {
  ...
  diaryText(clock) {
    ...
    // なつの おわりの 気配 (ただし この あとの「その日ならでは」の むすびが かてば そちらを つかう)
    ...
    if (s.today.some((t) => t.includes('なかなおり'))) closer = 'けんかしても、また ならんで しゃがめる。……そういうのを、ともだちって いうんやと おもう。';
    // さいごの日だけは、どんな一日でも この むすびに なる
    if (s.day >= 31) closer = 'あしたは、とうきょうに かえる。<br>……この なつやすみを、ぼくは ぜったいに わすれない。';
  ...
  async showEnding() {
    ...
    if (s.flags.kenkaDone) pages.push('ホームの ずっと むこうで、<br>ケンタと ミナが ならんで 手を ふっとる。<br><br>そえぎの ひまわりは、あれから ちゃんと 上を むいた。<br>ふたりが むすんだ ひもは、まだ ほどけとらん。');
