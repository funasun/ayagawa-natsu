...
export class EventSystem {
    ...
    if (this.world.indoor && this.world.sub === this.world.interior2f) {
      ...
      list.push({
        ...
        action: async () => {
          ...
          // 日中はひるねだけ (まる1日スキップは できない — 夜は ばんごはんを たべてから ねる)
          const pick = await this.ui.choice('まだ ひが たかいけど…', ['ひるねする (ゆうがたまで)', 'やめとく']);
          if (pick !== 0) return;
      ...
      list.push({
        ...
        action: async () => {
          ...
          const koshien = this.koshienLine(c.day);
          ...
          if (koshien && c.min >= 600 && c.min < 1140 && !s.flags['koshien' + c.day]) {
            s.flags['koshien' + c.day] = true;
            logEvent(s, 'ラジオで こうしえんの ちゅうけいを きいた');
          } else {
  ...
  // ラジオの こうしえん中継: 香川代表「あやうら高」が、なつと ともに かちすすんでいく
  koshienLine(day) {
    // 大会は 8/3ごろ〜8/20ごろ。あやうらは じりじりと かちあがり、けっしょうで おしくも やぶれる
    if (day === 3) return 'ザーッ…「かがわ代表・あやうら、1回戦とっぱ! 3たい1!」 えっ、うちのけん かったんや!';
    if (day >= 4 && day <= 5) return '「あやうら高、みごとな しろぼしスタート。エースの右うでが ひかります」';
    if (day >= 6 && day <= 7) return 'ザーッ…「えんちょう10かい、あやうら サヨナラ勝ちィ――!!」 うわっ、しびれる しあいや!';
    if (day >= 8 && day <= 9) return '「2回戦とっぱ。あやうら、ベスト16へ。こうこうやきゅうは、なつの まんなかです」';
    if (day >= 10 && day <= 12) return 'ザーッ…「あやうら、3回戦も 制す! ベスト8! かがわぜんいきが わいております!」';
    if (day >= 13 && day <= 15) return '「おぼんの あまぞらの した、あやうら じゅんじゅんけっしょう。……かった! ベスト4じゃ!」';
    if (day >= 16 && day <= 17) return 'ザーッ…「あやうら、じゅんけっしょう とっぱァ!! なんと けっしょうしんしゅつ!!」 しんじられん…!';
    if (day === 18) return '「いよいよ あす、けっしょうせん。あやうらのエース、この なつ さいごの マウンドです」';
    if (day === 19) return 'ザーッ…「けっしょうせん、2たい1。あやうら、あと ひとりで…… ―― うたれたァ。……ゲームセット」';
    if (day === 20) return '「あやうら、じゅんゆうしょう。ないた ないた、ナインが みんな ないとる。……ええ なつやったのう」';
    if (day === 21) return 'ザーッ…「こうしえんの つちを つめる きゅうじ。……ことしの なつも、おわりましたね」 …なんか さびしいな。';
    return null;
  }
  ...
  async dinner() {
    const s = this.state;
    const day = s.day;
    if (s.flags['dinner' + day]) return;
    s.flags['dinner' + day] = true;
    const menus = [
      { m: 'しょうゆまめと むぎめし', l: '「まめは はたけの にくやけんな。ようけ おあがり」' },
      { m: 'ひやしそうめんと きゅうりの もみもの', l: '「そうめんは のどごしや。ズルッと いくんが ええんよ」' },
      { m: 'アジの ひらきと なすの にびたし', l: '「なすは ばあちゃんの はたけの や。あまいやろ」' },
      { m: 'ちらしずしと おつゆ', l: '「きょうは なんとなく ごちそうの きぶんやってん」' },
      { m: 'カレーライス (じゃがいも ゴロゴロ)', l: '「こどもは みんな カレーが すきやけんな」' },
      { m: 'たきこみごはんと とうふの みそしる', l: '「おこげの ところ、あんたに あげよ」' },
      { m: 'てんぷらと そうめん', l: '「さぬきの てんぷらは あげたてが いちばんや」' },
      { m: 'にくじゃがと むぎちゃ', l: '「ようけ あそんだ かおしとるわ。おかわり あるで」' },
    ];
    ...
    if (day === 1) {
      ...
      pages = [
        `ちゃぶだいに ${day === 14 ? 'おはぎと そうめん' : 'おぼんの ごちそう'}が ならぶ。<br><br>「おぼんはな、ごせんぞさんが かえってくる ひなんよ。<br>じいちゃんも、きっと この いえに おるで」`,
        'ばあちゃんは そういって、ぶつだんの ごはんを<br>ちいさな おちゃわんに よそった。<br><br>ろうそくの ひが、ゆらゆら ゆれとる。',
      ];
    } else if (day === 20) {
      pages = [
        'たいふうで でんきが ちらちらする。<br>ろうそくの あかりで、おにぎりと つけものの ばんごはん。<br><br>「こんな ばんも、わるうないやろ」<br>あめの おとの なか、ばあちゃんが わらった。',
      ];
    } else if (day === 24) {
      pages = [
        `きょうは ${mm.m}。<br><br>「あしたは まつりやなあ。じいちゃんは まいとし<br>いちばん のりで でかけよったんよ」<br><br>なんだか こっちまで そわそわしてくる。`,
      ];
    } else if (day === 25) {
      pages = [
        'まつりの よるは、やたいの やきそばが ばんごはん。<br><br>ソースの におい。ちょうちんの あかり。<br>とおくで たいこの おと。<br><br>おなかも むねも いっぱいや。',
      ];
    } else if (day === 30) {
      pages = [
        `きょうは ${mm.m}。<br><br>「……あしたで さいごやなあ」<br>ばあちゃんは それだけ いうて、<br>いつもより ゆっくり おちゃを のんだ。`,
        'ぼくも なにか いおうと したけど、<br>ことばに ならんかった。<br><br>せんぷうきだけが、いつもどおり まわっとる。',
      ];
    } else {
      pages = [`ちゃぶだいに ばんごはんが ならぶ。<br>きょうは ${mm.m}。<br><br>${mm.l}`];
    }
    await this.ui.showStory(pages);
    logEvent(s, 'ばあちゃんとばんごはんをたべた');
  }
