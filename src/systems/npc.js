  ...
  async talk(npc) {
    ...
    if (npc.def.id === 'baachan' && !this.state.flags.metBaachan) {
      ...
      if (!this.state.flags.tutorial) {
        ...
        const pages = [];
        // タッチの ひとは 起動時の「そうさガイド」で ボタン説明ずみ → 重複ページは とばす
        if (!(t && this.state.flags.touchTut)) {
          pages.push(
        ...
        pages.push('この まちには、じかんが ながれとる。<br>あさ、ひる、ゆうがた、よる……<br>じかんや ひにちで、あえるひとや<br>おこることが かわっていくんや。<br><br><b>8がつ31にち</b>まで、<br>じゆうに なつやすみを すごそう。');
